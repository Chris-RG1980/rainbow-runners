...
### Framework
The entire site is built using Bootstrap 5, ensuring responsiveness and modern web standards. Bootstrap’s grid system and components have been utilised to create a consistent layout that adapts seamlessly across devices, enhancing the user experience.
...
- **Color**: Utilized to denote importance, categorise information, and direct user attention effectively.
...
- **Participation Rewards:** Implement a rewards system that acknowledges members for their participation and achievements. This system will recognize and incentivise active engagement and contributions within the club.
...
- **Profile Management**: Can update their profile and deactivate their account.
- **Shopping Basket**:
  - Can access club kit products in the store reserved for members only.
  - Can add products to their shopping basket.
  - Can edit the quantity of products in the basket.
  - Can delete products from the basket.
...
- **Shopping Basket**:
  - Has access to a limited amount of products.
  - Can add products to their shopping basket.
  - Can edit the quantity of products in the basket.
  - Can delete products from the basket.
...
For storing inquiries and questions submitted through the contact page, **MongoDB** has been chosen as the non-relational database solution. MongoDB provides the flexibility required for handling data that does not require relational integrity, which is ideal for the independent nature of user inquiries. This setup allows for scalable data storage that can adapt to varying loads and data types without predefined schemas.

### Relational Database

For all other data that benefits from structured relationships and integrity, such as user data, product information, and transaction records, I have chosen to use Postgresql which is hosted on Heroku in production and when developing locally SQLite is used instead. I have selected the free tier of **Heroku Postgresql** for its reliability and longevity, especially given that ElephantSQL, the previous provider, is reaching the end of its life. Postgres offers a robust, scalable, and efficient relational database solution, ensuring data integrity and seamless operations for all aspects of the site that require relational data handling.
***
## Database Schema
...
- [Django-bleach](https://django-bleach.readthedocs.io/) -  Integrates the Bleach library to sanitise HTML.         
- [Django-CKEditor](https://django-ckeditor.readthedocs.io/) - Integrates the CKEditor text editor.      
